#! /usr/bin/env python
#SBATCH --job-name=mellon_stability_d_noNy
#SBATCH --nodes=1
...
BASE_OUT_PATH = '/fh/fast/setty_m/user/dotto/CellDensities/data/benchmarks/subsamples/stability_d_noNy/'
...
def make_estimator(sub_id, ad, n_landmarks=5000):
    ...
    if odd:
        number = -number
    factor = 1.05**number
    d = ad.obsm['DM_EigenVectors'].shape[1] * factor
    
    est = mellon.DensityEstimator(n_landmarks=n_landmarks, d=d, rank=1., method='percent')
    return est
