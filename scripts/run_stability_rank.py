#! /usr/bin/env python
#SBATCH --job-name=mellon_stability_rank
#SBATCH --nodes=1
...
BASE_OUT_PATH = '/fh/fast/setty_m/user/dotto/CellDensities/data/benchmarks/subsamples/stability_rank/'

def make_estimator(sub_id, X, n_landmarks=5000):
    rank = .99997 ** (sub_id**2)
    est = mellon.DensityEstimator(n_landmarks=n_landmarks, rank=rank, method='percent')
    return est
